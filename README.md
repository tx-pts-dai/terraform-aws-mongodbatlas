...
| <a name="input_ip_access_list"></a> [ip\_access\_list](#input\_ip\_access\_list) | List of allowed IPs to the project<br/><br/>    ip     : can be either an IP address or CIDR block<br/>    comment: useful information to identify the IP | <pre>list(object({<br/>    ip      = string<br/>    comment = string<br/>  }))</pre> | `[]` | no |
